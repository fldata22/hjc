...
// === Top bar ===
export const TopBar = ({ eyebrow, title, back, right, transparent }) => {
  const navigate = useNavigate();
  return (
    <div className="mw-topbar" style={transparent ? { background: 'transparent', borderBottom: 'none' } : null}>
      <div className="mw-topbar-l">
        {back ? (
          <button className="mw-iconbtn" aria-label="Back" onClick={() => navigate(-1)}>
  ...
  return (
    <div className="mw-tabbar">
      {tabs.map(t => {
        if (t.center) {
          return (
            <div key={t.key} className="mw-tabitem" style={{ justifyContent: 'flex-start', paddingTop: 2 }} onClick={() => navigate(t.to)}>
        ...
        return (
          <div key={t.key} className={`mw-tabitem ${active === t.key ? 'active' : ''}`} onClick={() => navigate(t.to)}>
...
export const Phone = ({ active, children, top, fab, noPad, gray }) => (
